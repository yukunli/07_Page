...
在实验室条件下，本文采用干燥法对水分测量系统进行标定。然后对含水量为~3\%$\sim$8\%~的烧结混合料进行了水分测量实验。实验表明，仪器的测量距离为~0$\sim$150mm~时，仪器的最大绝对测量误差为~0.33\%~水分含量，重复性相对标准偏差为~2.15\%。
...
In the laboratory conditions, this paper uses the drying method to calibrate the moisture measurement system. Then the moisture content of 3\% to 8\% of the sintering mixture for moisture measurement experiments. The experimental results show that the maximum absolute measurement error is 0.33\% water content and the relative standard deviation is 2.15\% when the measuring distance is 0$\sim$150mm.
