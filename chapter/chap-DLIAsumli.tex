...
\section{锁相放大器}
锁相放大器，是以相关检测技术为基础，利用参考信号频率与待测信号频率相关，与噪声不相关的特性，进而从较强的噪声背景中提取出有用信号的一种微弱信号检测装置\upcite{c3-tongbu2}。锁相放大器需要采用一定频率的参考信号作为比较的基准，该参考信号只能是与被测信同频（或倍频）、同相的信号，而与大部分噪声分量没有相关性。所以当参考信合和被测信号做相关运算后，噪声信号会被滤除掉，从而能极大的抑制噪声，提高检测信噪比。
...
而DLIA~是将~ALIA~中的相敏检波器由~DSP~或微处理器内部编程实现，低通滤波器由数字低通滤波器实现的一种数字解调运算，最后将待测信号的幅值、相位结果输出。
