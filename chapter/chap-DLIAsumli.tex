...
锁相放大器虽然可以很好的检测强噪声背景中的微弱信号，但却对参考信号的相位有着严格的要求。待测信号和参考信号的相位差为~$0^{\circ}$~时，锁相放大器的直流输出信号才能真实地反映实际信号的变化。虽然可以通过参考信号和被测信号之间加入移相电路来使两者的相位差为~$0^{\circ}$，但是，在实际的应用中，要使两者之间的相位差保持为~$0^{\circ}$~是相当困难的。基于上述原因，在实际应用中一般使用两个相敏检波器组成正交矢量型锁相放大器。正交矢量型锁相放大器可以很好地避免相位差对测量结果的影响，从而能够获得更好的信噪改善比。
...
以上运算便是数字锁相放大器的核心算法原理，在微处理器的内部通过编程实现。从上述过程可以看出，数字锁相放大器的相关算法需要进行大量的乘累加运算，运算量的大小与待测信号的序列长度成正比。同时，数字低通滤波器也要进行大量的卷积运算，这也将消耗一部分运算的时间。所以，数字锁相放大器在实现通过数字信号处理方式进行微弱信号检测的同时，对微处理器的运算速度和运算时间也提出一定的要求，一般采用~DSP~微处理器实现。
...
（3）理想的相敏检波是建立在积分过程中积分时间无限长的基础上的，但实际测量中，电容自身的漏电现象使得积分时间变短，所以对于缓变的周期信号，模拟锁相放大器的检测效果变差。
...
（3）数字锁相放大器调试方便，灵活性好。同时可以做成闭环系统，根据数字相关器的处理结果实时地计算出有用信号的幅值和相位角，能够进行自动调节增益、自动改变参考信号相位等操作\upcite{c3-da3}。
...
~NumPy、SciPy、Matplotlib、Enthought librarys~等众多函数库的开发完善，Python~已经具备了足够的功能，能够充分满足常见的科学计算需求\upcite{c3-py1}。和科学计算领域最流行的商业软件~MATLAB~相比，Python~是一门通用的程序设计语言，比~MATLAB~所采用的脚本语言的应用范围更广泛，有更多的程序库的支持，完全免费并且开放源码。Python~适用于~Windows~和~Linux~等多种平台，在数字信号处理领域中的应用也越来越普及。
\begin{figure*}[!ht]
\centering
...
Python的整体框架如图~\ref{fig4_0}~所示，主要由三个组成部分:
...
数字锁相放大器的性能指标主要有测量精度和频率分辨能力。由文献\cite{c3-wx1}可知，要提高数字锁相放大器的测量精度和频率分辨能力，应加大总的测量时间。但在测量时间一定的情况下，要加大数据长度就需要提高信号采样频率，而提高了信号采样频率会使数字相关运算的时间减小，所以合理的选择采样频率和采样点数是非常重要的。

\subsection{仿真结果与分析}

根据本课题中探测器接收到的信号的频率，本文以待测信号为~5kHz~为例进行仿真。根据奈奎斯特定理，采样频率必须大于２倍的待测信号的频率。为了避免由非周期采样给数字锁相运算结果带来误差，应尽量使采样频率与待测信号的频率的比值为一个整数。
