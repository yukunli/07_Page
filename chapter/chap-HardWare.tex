...
高阻抗前置放大器是将光电探测器等效为一个高内阻的信号源，然后与一个高阻抗的负载电阻进行匹配以获得最大增益。高阻抗前置放大器因具有噪声小、增益高等优点而被广泛应用。但它需要在主放大器中设计一个复杂的均衡网络来消除由光电探测器的偏置电阻和场效应管的输入电容对电流信号形成的积分效应。高阻抗前置放大器一般用于混合集成电路，对速率要求不高但对灵敏度要求很高的检测系统有较好的效果\upcite{c4-kd1}。

跨阻前置放大器的结构也简单，只需要在输入端和输出端之间跨接一个反馈电阻，形成电压并联负反馈，就可以使得电路增益稳定，动态范围增大，频带展宽。并且它不需要均衡电路。基于这些优点，跨阻放大器广泛用于高速传输系统。
...
InGaAs~光电探测器是基于光生伏特效应的~PV~光电二极管，将掺杂浓度很低的本征半导体加入其~PN~结中间，在加厚的耗尽层区就会形成强电场区。当入射光照射在光电探测器上时，本征层将入射光子全部吸收，而光生载流子则在强电场作用下加速运动至~N~层。由于载流子渡越时间非常短，同时耗尽层的加厚使结电容大幅减小，因此光电探测器就获得了较高的响应速度\upcite{c2:tp4}。InGaAs~光电探测器可等效成如图~\ref{fig57}~所示电路，由一个旁路电阻~$R_{D}$~ 和旁路电容~$C_{D}$~组成的电流源。图~\ref{fig56}~也是一个的电流-电压转换电路，输出电压~$V_{out}=I_{in}\times R_{f}$。
...
由于本课题采用的光电探测器内阻比较小，$R_{D}\approx 1K\Omega$，所以设计优良的低噪声前置放大器变的非常重要。为获得较低的放大器噪声和较高的灵敏度，反馈电阻~$R_{f}$~要比探测器内阻大很多，本电路中~$R_{f}$~取值~$1M\Omega$。

运算放大电路的输入电容和负载电容（还包括耦合电容和旁路电容）会导致相位滞后，当相位滞后到~$180^{\circ}$~时，电路由负反馈变成了正反馈电路，从而引起电路震荡。因为电路震荡是由于放大器的输入电容和负载电容造成相位裕度变小而引起的，所以要消除震荡，需要用相位补偿，就是使电路的相位裕度保持在~$45^{\circ}\sim60^{\circ}$~范围内。通常采用将补偿电容加在反馈回路的方式进行超前相位补偿。图~\ref{fig58}~中的电容~$C_{s}$~ 就是超前补偿电容。它可以抵消输入电容和复杂电容的极点，产生新的零点和极点。在高增益和大反馈电阻的放大电路中，电容~$C_{s}$~的取值计算公式为：
...
\begin{figure*}[!h]
\centering
\includegraphics[width=11cm]{fig48.pdf}
...
为了提高~AD7656~的采集转换信号的精度，避免~AD7656~的输入端对信号源信号分压，所以设计阻抗匹配电路是非常必要的。在~AD7656~前级输入加入由运算放大器构成的电压跟随器很够获得较低的输出阻抗，从而驱动~AD7656~的输入端，电路原理如图~\ref{fig50}~所示。AD7656~ 输入端具有容性负载特性，通常采用一个电阻和电容组成外部补偿电路用以减少容性负载的影响。同时，电阻和电容还可以做低通滤波器，实现抗混叠滤波电路的效果。
