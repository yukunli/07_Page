...
TMS320F2000~系列~DSP~融合了控制外设的集成功能与微处理器（MCU）的易用性，具有强大的控制和信号处理能力以及~C~语言编程效率。TMS320F28335~是在已有的~DSP~平台上增加了浮点运算内核，能执行复杂的浮点运算，可以节省代码执行时间和存储空间，具有精度高、成本低、功耗小、外设集成度高、数据和程序存储量大等优点，可以为嵌入式工业应用提供更加优秀的性能和更加高效的软件设计。TMS320F28335~的主要特点总结如下：
...
\begin{figure*}[!h]
\centering
\includegraphics[width=4cm]{fig57.png}
\caption {探测器等效电路}
\addtocounter{figure}{-1}
\renewcommand\figurename{Fig.}
...
根据式~\ref{xd15}$\sim$\ref{xd18}~可知，带通滤波器的中心频率与电容~C~的值、电阻~R3~的值以及~R1和~R2~的并联值有关，电阻~R3~的变化会同时影响中心频率、增益和品质因素。R1、R2~ 的并联值影响中心频率和品质因数。R2~不影 响通带增益。由通带宽的计算公式知，通带宽与电容~C~ 和电阻~R3~成反比\upcite{}。

本课题中的三路信号频率分别为~4kHz，6kHz，8kHz，经过信号放大电路的信号需要接入窄带带通滤波器将三路信号分别滤出来。德州仪器的~FilterPro~程序可以根据所要求的滤波器指数设计出满足要求的滤波器。本题题使用该软件进行窄带带通滤波器的设计，以通频带中心频率为~6kHz~为例进行说明。

打开~FilterPro~应用软件，输入设计所需的新能参数，选取中心频率~$f_{0}=6000Hz$，允许的带通纹波为~0.5dB。通带带宽为~500Hz，角频率衰减为~-3dB。同时选择滤波器阶次固定为~4~阶，最后设计的巴特沃斯带通滤波器如图~\ref{fig62}~所示。
\begin{figure*}[!h]
\centering
...
AD7656~的~6~路模拟信号输入通道都是相关独立的，外部模拟信号从~V1$-$V6~引脚输入，经过一个高带宽的线性跟随器（~T/H~）单元进行采样。每路~ADC~ 均按照逐次逼近原理进行，由控制逻辑电路和~SAR~模数转换器共同完成。当模拟信号转换为数字信号后，由控制逻辑单元控制数字输出驱动电路以串行或并行数据的方式输出。
...
工作环境温度的变化对光电探测器和~LED~光源性能的好坏有重要影响，当温度不在探测器的工作温度时会使光电探测器的暗电流增大，同时探测器的噪声会变大，探测器在近红外谱段的灵敏度也会降低。而温度的剧烈变化也会引起~LED~光源发出的近红外波段发生偏移。为了使探测器和~LED~光源保持良好的工作性能，必须对其工作环境温度实现控制。本课题采用数字温度传感器~DS18B20~进行温度采集，通过~PID~控制算法，利用~PWM~脉宽调制技术驱动半导体制冷，从而实现对温度的调节。
...
\subsection{半导体制冷电路}
半导体材料具有最佳的热点能量转换特性，给半导体材料通过直流电时具有制冷功能，这是半导体制冷的原理。与常规的机械制冷相比，半导体制冷具有无运动部件、体积小、制冷速度快、调节性能好和易于微型化等优点，在一些场合有着别的制冷方式无法替代的作用。介于本课题对制冷器安装空间的限制，采用微型半导体制冷片制冷方式对探测器温度进行控制。
...
\begin{figure*}[!h]
\centering
\includegraphics[width=14cm]{fig52.png}
\caption {半导体制冷电路}
\addtocounter{figure}{-1}
\renewcommand\figurename{Fig.}
...
PWM~控制方式是靠改变脉冲宽度来控制输出电压，通过改变脉冲信号占空比就可以实现输出电压的精确调制，能够很好避免其他方式容易超调的情况出现。制冷电路部分与数字部分使用的是不同的电源，为避免互相干扰，采用高速光耦~6N137~进行隔离。6N137~芯片的光电开关切换频率高达~10MBit/s，能够实现~PWM~信号的精密整形。PWM~的输出电压通过一个简单的~V/I~变换，将直流信号转换为电流信号，该电流就是流过半导体制冷片的电流。如此，通过控制~PWM~输出方波的占空比，就能够精确调节流过半导体制冷片的电流，从而控制制冷片的制冷功率。
