...
本课题设计的水分测量系统主要由以下四部分组成：光源激励模块、信号处理模块、DSP~控制模块以及电源电路模块。其中信号处理模块由模拟信号处理模块和数字锁相放大器组成，数字锁相放大器由~DSP~微控制内部实现，所以在硬件电路的实现上，信号处理模块主要是指模拟信号处理。硬件电路的主要功能是为~LED~光源提供激励信号，并对光电探测器接收到的电信号进行放大、滤波、采集等数据处理，并将采集到的信号传送到微控制器进行后续数据处理。硬件系统的整体结构框图如图~\ref{fig40}~所示。
\begin{figure*}[!ht]
\centering
...
为了简化电源电路设计的复杂性，本课题选用了金升阳的~LH~系列~AC-DC~电源模块，LH~系列电源模块输入电压范围为~85$\thicksim$264VAC，50$\thicksim$60Hz，输出电源稳定、低纹波、低噪声，具有输出短路、过流和过温保护功能。该系列的电源模块只需要几个电容电阻就能实现~AC-DC~的精确输出，各输出电压的功率也能满足本课题的要求。
...
D1、D2~为瞬变电压抑制二极管，它是一种高性能的电路保护器件，具有承受瞬间大电流的能力，当电路中有瞬变电压发生时，该二极管能够快速地耗散掉很大的瞬变电流，将电路电压箝位于低电压上，从而保护后面的电路。

本课题中数字电路的核心是~DSP~芯片~TMS320F28335~处理器。为满足该~DSP~芯片内部功能模块的不同供电需求，设计了幅值为~3.3V~和~1.9V~的数字电源和模拟电源两种。数字电源电路采用了~DSP~专用电源芯片~PS767D301，能够同时输出两种电压，一路固定输出~3.3V，一路可以在~1.5V-5.5V~范围内调整。
...
~R43~分压反馈到~24~引脚，使得输出电压稳定在~1.9V，这样便得到数字电源~3.3V~和~1.9V。再分别通过电感~L44~和~L45~得到模拟电源~3.3V~和~1.9V。C44$\sim$C51~均为滤波电容，0.1$\mu$F~电容可以滤除高频干扰，22$\mu$F~电容可以滤除低频干扰，使得电源输出更稳定。
...
光源信号驱动电路为近红外~LED~光源提供精密稳定地按正弦规律变化的驱动电流。LED~光源的工作电压低，效率高，驱动电流过大会损坏电源，过小又会降低光源发出的光强。驱动电路不稳定也会影响整个仪器的测量精度，因此光源信号驱动电路的设计对整体系统的实现至关重要。
\subsection{激励信号发生器}
...
DSP~微控制器通过三线制~SCI~通信方式对~AD9833~的输出频率进行控制，其外部参考时钟由频率为~1MHz~的有源晶振生产。AD9833~的输出信号的峰峰值的典型值为~0.6V，而且是单极性波形，经过简单的信号幅值放大后就能够满足驱动后续恒流源电路的设计要求。由于~AD933~产生的波形直接由内部的~DAC~输出，芯片内部不含滤波器，为了使后续驱动~LED~光源的信号频率更单一，在~AD9833~ 的波形输出端设计了一个~RC~低通滤波器。

本课题需要三种不同频率的正弦激励信号，而每路正弦波发生电路只能产生一种固定频率的正弦信号，所以在硬件电路上，本课题分别设计了三路相对独立的正弦波发生电路用来输出不同频率的正弦周期信号。三路~AD9833~共用同一个有源晶振作为外部参考时钟，在与~DSP~微控制器通讯的接口设计上，三路~AD9833~芯片也采用相同的~SCI~数据线和时钟线接口，DSP~通过控制~AD9833~的片选信号线对三路~AD9833~进行选通。同样的道理，三路~AD9833~的波形输出也将分别独立的进入到后续的信号放大电路和恒流源电路中。

\subsection{恒流源驱动电路}

LED~光源作为近红外水分测量系统中光路的第一个部件，其发出的光信号的精度和稳定性影响着整个系统的精度和稳定性。同时，为了使~LED~光源产生足够大光强的近红外光，根据~LED~光源的电流特性，本文设计的恒流源激励电路提供给~LED~光源~$0\sim200mA$~的正弦变化的激励电流。在设计恒流源电路时，不仅要考虑该激励电流的稳定性，也要考虑瞬变的大电流、反向电压和静电等会损坏光源的情况。本课题设计的恒流源电路如图~\ref{fig46}~所示。
\begin{figure*}[!ht]
\centering
...
在压控恒流源的设计中，精密电阻和限流电阻的取值非常关键，如果电阻取值不当会造成流过负载的电流信号失真的情况。本课题将恒流源电路简化成如图~\ref{fig47}~所示的等效模型进行分析。这个电路实质是一个负反馈电路，通过~R~的电流为~$I_{R}$，其端电压为~$U_{b}$，流过~$R_{L}$~ 和~$R_{o}$~ 电路为~$I_{R_{L}}$，由运放两输入端“虚短”关系有：
...
本课题中恒流源电路的输入是幅值约为~$0\sim$4V~正弦电压信号，而驱动~LED~光源的正弦电流激励的幅值约为为~$0\sim200$mA，所以精密电阻~$R$~取值~$20\Omega$，且功耗要在~1W~左右。LED~光源在一定激励电流下阻值约为~$24\Omega$，考虑到恒流源电路的电源~$V_{CC}$~ 为~15V，所以电阻~$R_{o}$~ 取值~$40\Omega$~ 比较合适。
