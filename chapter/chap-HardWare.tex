...
本课题采用~DALLAS~半导体公司生产的~DS18B20~温度传感器，其测温范围为~-55$^{\circ}$C~$\sim$~+125$^{\circ}$C，测温分辨率可达~0.0625$^{\circ}$C。 它具有微型化、低功耗、抗干扰能力强、易与微处理器接口等优点。它集温度测量，A/D~转换于一体，只需一根端口线就能与单片机通讯，测量系统电路简单，易于实现。 温度测量电路如图~\ref{fig51}~ 所示。
\begin{figure*}[!h]
\centering
\includegraphics[width=9cm]{fig51.pdf}
\caption {测温电路}
\addtocounter{figure}{-1}
\renewcommand\figurename{Fig.}
